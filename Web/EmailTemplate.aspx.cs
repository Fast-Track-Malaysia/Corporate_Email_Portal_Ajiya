            ...
            try
            {
                using (SqlConnection conn = new SqlConnection(Session["ConnString"].ToString()))
                {
                    if (Mode.Text == "Add")
                    {
                        ...
                        {
                            dat.SelectCommand.CommandText = "SELECT * FROM ft_email_template_statement WHERE Format = '" + txtFormat.Text + "'";
                        ...
                        string addTemplate = "INSERT INTO ft_email_template_statement (Format, EmailSubject, EmailContent, LastUpdateUser, LastUpdateTime, isDefault) VALUES ('" +
                    ...
                    if (Mode.Text == "Edit")
                    {
                        string updateTemplate = "UPDATE ft_email_template_statement SET EmailSubject = '" + txtSubject.Text.Replace("'","") + "', EmailContent = '" + EmailEditor.Content.Replace("'", "") + 
                    ...
                    {
                        string setDefTemplate = "UPDATE ft_email_template_statement SET isDefault = '0' " +
                            "UPDATE ft_email_template_statement SET isDefault = '1' WHERE Format = '" + txtFormat.Text + "'";
            ...
            try
            {
                if (e.CommandName == "E")
                {
                    ...
                    using (SqlConnection conn = new SqlConnection(Session["ConnString"].ToString()))
                    {
                        using (SqlDataAdapter dat = new SqlDataAdapter("", conn))
                        {
                            dat.SelectCommand.CommandText = "SELECT * FROM ft_email_template_statement WHERE ID = '" + id + "' ";
                            DataTable dt = new DataTable();
                            dat.Fill(dt);
                ...
                if (e.CommandName == "D")
                {
                    string deleteTemplate = "DELETE FROM ft_email_template_statement WHERE ID = '" + id + "'";
            ...
            using (SqlConnection conn = new SqlConnection(Session["ConnString"].ToString()))
            {
                using (SqlDataAdapter dat = new SqlDataAdapter("", conn))
                {
                    dat.SelectCommand.CommandText = "SELECT T0.ID, T0.Format, T0.EmailSubject, T0.EmailContent, " +
                        ...
                        "FROM ft_email_template_statement T0 ORDER BY T0.ID ";
                    DataTable dt = new DataTable();
                    dat.Fill(dt);
