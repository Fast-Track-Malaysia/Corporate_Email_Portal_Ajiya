...
namespace Web
{
    public partial class EmailSendReceipt : System.Web.UI.Page
    {
        ...
        protected static List<ReceiptSelectedRow> ReceiptSelectedRow = new List<ReceiptSelectedRow>();
        protected static List<ReceiptSelectedRow> ReceiptSelectedRowAll = new List<ReceiptSelectedRow>();
        ...
        protected void btnSelect_Click(object sender, EventArgs e)
        {
            ...
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                ReceiptSelectedRow.Add(new ReceiptSelectedRow
                {
                ...
                LabelInfo.Text = "Selected : " + ReceiptSelectedRow.Count();
            }
        }
        ...
        protected void btnSearch_Click(object sender, EventArgs e)
        {
            ...
            warning = "";
            info = "Search...";
            ft_logs.WriteLine("Perform search for Receipt.", "Info");
            init();
            ReceiptSelectedRow.Clear();
            ...
            try
            {
                ...
                using (SqlConnection conn = new SqlConnection(Session["ConnString"].ToString()))
                {
                    ...
                    {
                        query = "SELECT * FROM [FTS_fn_GetBPList_Receipt] ('" + DateFromStr + "', '" + DateToStr + "', '"
        ...
        protected void MainTask()
        {
            ...
            ft_logs.WriteLine("Receipt Date From:  " + txtDateFrom.Text, "");
            ft_logs.WriteLine("Receipt Date To:  " + txtDateTo.Text, "");
            ft_logs.WriteLine("-------------------------------------------------------------------------------", "");
            // Core Function
            GenerateDocument();
            SendEmail();
        ...
        protected void GenerateDocument()
        {
            ...
            folderDir = "C:\\EmailPortal\\[" + LabelCompany.Text + "] " + lblUser.Text + "\\Receipt\\" + todaydateStr;

            try
            {
                ft_logs.WriteLine("① Clear attachment folder...", "Info");
                foreach (string DocEntry in ReceiptSelectedRow.Select(x => x.DocEntry.ToString()).Distinct())
                {
            ...
            try
            {
                ...
                for (int i = 0; i < ReceiptSelectedRow.Count; i++)
                {
                    Checked = ReceiptSelectedRow[i].isChecked;

                    if (Checked)
                    {
                        int DocEntry = ReceiptSelectedRow[i].DocEntry;
                        string DocNum = ReceiptSelectedRow[i].SAPDocNo;
                        string CardCode = ReceiptSelectedRow[i].CardCode;
                        string CardName = ReceiptSelectedRow[i].CardName;
                        ...
                        info = "③ Generate Receipt [" + (i + 1).ToString() + "/" + ReceiptSelectedRow.Count() + "] ➜ [" + CardCode + "-" + CardName + "-" + DocNum + "]";
                        ...
                        string ExportPath = path + "RCT_" + CardCode + "_" + DocNum + ".pdf";
        ...
        protected void SendEmail()
        {
            ...
            using (SqlConnection conn = new SqlConnection(Session["ConnString"].ToString()))
            {
                using (SqlDataAdapter dat = new SqlDataAdapter("", conn))
                {
                    dat.SelectCommand.CommandText = "SELECT * FROM ft_email_template_receipt WHERE isDefault = '1' ";
            ...
            for (int i = 0; i < ReceiptSelectedRow.Count; i++)
            {
                ReceiptSelectedRow receiptSelectedObj = ReceiptSelectedRow[i];
                Checked = receiptSelectedObj.isChecked;

                if (Checked)
                {
                    string DocEntry = receiptSelectedObj.DocEntry.ToString();
                    string DocNum = receiptSelectedObj.SAPDocNo;
                    DateTime DocDate = receiptSelectedObj.DocDate;
                    string CardCode = receiptSelectedObj.CardCode;
                    string CardName = receiptSelectedObj.CardName;

                    string to = receiptSelectedObj.EmailTo;
                    string cc = receiptSelectedObj.EmailCC;

                    info = "④ Sending Receipt [" + (i + 1).ToString() + "/" + ReceiptSelectedRow.Count() + "] ➜ [" + CardCode + "-" + CardName + "-" + DocNum + "] TO: [" + to + "] CC: [" + cc + "]";
                    ft_logs.Write(info, "Info");
        ...
        protected void btnExportPDF_Click(object sender, EventArgs e)
        {
            ...
            string ExportPath = Path.GetTempPath() + @"Combined_RCT @ " + statementdate.ToString("MMM\\'yy") + ".pdf";
            int i = 0;
            while (System.IO.File.Exists(ExportPath))
            {
                ExportPath = Path.GetTempPath() + @"Combined_RCT @ " + statementdate.ToString("MMM\\'yy") + "[" + (i++).ToString() + "].pdf";
            }
        ...
        protected void chkAll_CheckedChanged(object sender, EventArgs e)
        {
            ...
            ReceiptSelectedRow = new List<ReceiptSelectedRow>();
            if (chkAll.Checked)
            {
                ReceiptSelectedRow = ReceiptSelectedRowAll;
            }
            else
            {
                ReceiptSelectedRow.Clear();
        ...
        protected void chkBox_CheckedChanged(object sender, EventArgs e)
        {
            ...
            {
                ReceiptSelectedRow.RemoveAll(x => x.DocEntry == docentry);
            }
            ReceiptSelectedRow.Select(x => x.DocEntry).Distinct();
            LabelInfo.Text = "Selected : " + ReceiptSelectedRow.Count();
        }
        // -- Web Page Rendering -----------------------------------------------------------------------
        ...
        protected void GridView1_PreRender(object sender, EventArgs e)
        {
            ...
            ReceiptSelectedRowAll = new List<ReceiptSelectedRow>();
            ...
            foreach (GridViewRow row in GridView1.Rows)
            {
                ...
                ReceiptSelectedRowAll.Add(new ReceiptSelectedRow
                {
                    DocEntry = int.Parse(lblDocEntry.Text),
                    SAPDocNo = lblSAPDocNo.Text,
                    DocDate = DateTime.ParseExact(lblDocDate.Text, "dd/MM/yyyy", null),
        ...
        protected void GridView1_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            ...
            { 
                if (ViewState["chkAll"] != null)
                {
                    ...
                    if (ReceiptSelectedRow.Count() == ReceiptSelectedRowAll.Count())
                    {
                        chkAll.Checked = true;
                    }
                }
        ...
        protected void GridView1_DataBound(object sender, EventArgs e)
        {
            ...
            for (int i = 0; i < GridView1.Rows.Count; i++)
            {
                ...
                // ChkBox Rendering
                if (ReceiptSelectedRow.Exists(x => x.DocEntry == docentry) && !chkBox.Enabled)
                {
                    ReceiptSelectedRow.RemoveAll(x => x.DocEntry == docentry);
                    ReceiptSelectedRowAll.RemoveAll(x => x.DocEntry == docentry);
                    chkBox.Checked = false;
                }
                else if (ReceiptSelectedRow.Exists(x => x.DocEntry == docentry) && chkBox.Enabled)
                {
                    chkBox.Checked = true;
                }
                else
            ...
            if (ReceiptSelectedRow.Count() > 0)
            {
                LabelInfo.Text = "Selected records : " + ReceiptSelectedRow.Count();
            }
        }
