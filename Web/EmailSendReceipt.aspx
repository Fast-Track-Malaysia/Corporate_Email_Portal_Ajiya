<%@ Page Title="E-Mail Receipt" Language="C#" MasterPageFile="~/Site.Master" AutoEventWireup="true" CodeBehind="EmailSendReceipt.aspx.cs" Inherits="Web.EmailSendReceipt" MaintainScrollPositionOnPostback="true" %>
...
<asp:Content ID="Content1" ContentPlaceHolderID="ContentPlaceHolder1" runat="server">
    ...
    <h1 class="font_bold wrap">E-Mail Receipt</h1>
    ...
    <asp:SqlDataSource CancelSelectOnNullParameter="true" ID="SqlDataSource" runat="server"
        SelectCommand="SELECT * FROM [FTS_fn_GetBPList_Receipt] (@DateFromStr, @DateToStr, @strFr, @strTo) T0 ORDER BY T0.DocEntry "
            ...
            <div class="table-responsive">
                <asp:GridView ID="GridView1" runat="server" class="table table-striped table-bordered table-hover"
                    ...
                    <Columns>
                        ...
                        <asp:TemplateField HeaderText="SAP Doc No." SortExpression="SAPDocNo" ItemStyle-Width="150px">
                            <ItemTemplate>
                                <asp:Label ID="lblSAPDocNo" runat="server" Text='<%# Bind("SAPDocNo") %>'></asp:Label>
                            </ItemTemplate>
                        </asp:TemplateField>
                        <asp:TemplateField HeaderText="Posting Date" SortExpression="DocDate" ItemStyle-Width="150px">
                            <ItemTemplate>
                        ...
                        <asp:TemplateField HeaderText="View" HeaderStyle-CssClass="text-center">
                            <ItemTemplate>
                                <asp:HyperLink ID="LinkView" runat="server" CssClass="btn btn-primary btn-xs"
                                    Target="_blank"
                                    NavigateUrl='<%# string.Format("~/PreviewReceipt.aspx?DocEntry={0}&SAPDocNo={1}&CardCode={2}&Mode=View", HttpUtility.UrlEncode(Eval("DocEntry").ToString()), HttpUtility.UrlEncode(Eval("SAPDocNo").ToString()), HttpUtility.UrlEncode(Eval("CardCode").ToString())) %>'
                                    Text="Preview" />
                                <asp:HyperLink ID="LinkDownload" runat="server" CssClass="btn btn-primary btn-xs"
                                    NavigateUrl='<%# string.Format("~/PreviewReceipt.aspx?DocEntry={0}&SAPDocNo={1}&CardCode={2}&Mode=Download",HttpUtility.UrlEncode(Eval("DocEntry").ToString()), HttpUtility.UrlEncode(Eval("SAPDocNo").ToString()), HttpUtility.UrlEncode(Eval("CardCode").ToString())) %>'
                                    Text="Download" />
                            </ItemTemplate>
